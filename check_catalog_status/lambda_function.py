...
def lambda_handler(event, context):
    ...
    print(f"[CHECK_CATALOG_STATUS] Extracted from event:")
    print(f"  - catalog_name: {catalog_name}")
    print(f"  - jwt_token: {'<present>' if jwt_token else '<MISSING>'}")
    print(f"  - local_catalog_id: {local_catalog_id}")
    ...
    if not jwt_token:
        print("[ERROR] Missing jwt_token in event - cannot authenticate with external API")
        return {
            'success': False,
    ...
    print(f"[CHECK_CATALOG_STATUS] Authorization header: Bearer {jwt_token[:20]}..." if len(jwt_token) > 20 else f"[CHECK_CATALOG_STATUS] Authorization header: Bearer {jwt_token}")
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"[CHECK_CATALOG_STATUS] FULL API RESPONSE:")
